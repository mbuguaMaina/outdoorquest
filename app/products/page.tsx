      ...
      <section className="py-8 border-b border-border sticky top-20 bg-background/95 backdrop-blur-sm z-40">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            {/* Category Tabs */}
            <Tabs value={activeCategory} onValueChange={setActiveCategory} className="w-full overflow-x-hidden rounded-full md:w-auto">
              <TabsList className="grid grid-cols-5 gap-1 bg-muted p-1  w-max overflow-x-auto">
                ...
                <TabsTrigger value="educational" className="rounded-full data-[state=active]:bg-secondary data-[state=active]:text-secondary-foreground">
                  📚 Learn
                </TabsTrigger>
                <TabsTrigger value="outdoor" className="rounded-full data-[state=active]:bg-accent data-[state=active]:text-accent-foreground">
                  🌳 Outdoor
                </TabsTrigger>
                <TabsTrigger value="plush" className="rounded-full data-[state=active]:bg-pink data-[state=active]:text-pink-foreground">
                  🧸 Plush
                </TabsTrigger>
                <TabsTrigger value="general" className="rounded-full data-[state=active]:bg-orange data-[state=active]:text-orange-foreground">
                  🎯 General
                </TabsTrigger>
              </TabsList>
            </Tabs>
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`${colors.bg}/20 p-8 flex items-center justify-center`}>
                      <span className="text-6xl group-hover:animate-bounce transition-transform">
                        {toy.emoji}
                      </span>
                    </div>
                    ...
                    <div className="p-4 space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        <span className={`text-xs font-semibold px-2 py-1 rounded-full ${colors.bg}/10 ${colors.text} capitalize`}>
                          {toy.category}
                        </span>
                      ...
                      <p className="text-sm text-muted-foreground flex items-center gap-2">
                        <span>👶</span>
                        Ages {toy.ageRange} years
                      </p>
                      
                      {/* View Button */}
                      <Button asChild className={`w-full mt-2 btn-playful ${colors.bg}/10 ${colors.text} hover:${colors.bg} hover:text-white`}>
                        <a href={`https://wa.me/+254758929927?text=Halo%20OutDoorQuest,do you still have this toy ${toy.name}?`} target="_blank" rel="noopener noreferrer">
                          Buy on Whatsapp
                        </a>
                      </Button>
                    </div>
